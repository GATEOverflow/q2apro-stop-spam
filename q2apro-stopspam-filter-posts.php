...
class q2apro_stopspam_filter_posts
{
	...
	public function filter_email(&$email, $olduser)
	{
		...
		if(!empty($email) && !empty($emailbans))
		{
			// array from string
	...
	function filter_answer(&$answer, &$errors, $question, $oldanswer)
	{
		...
		$dom = new DOMDocument();
		$html = $answer['content'];
		$html = preg_replace('/((www|http:\/\/|https:\/\/)[^ ]+)/', '<a href="\1">\1</a>', $html);
		$xml = preg_replace('#<a.*?>.*?</a>#i', '', $html);
		$xml = strip_tags($xml);

		if(str_word_count($xml) < $linkcount/2)
		{
			$errors['content'] = qa_lang('q2apro_stopspam_lang/link_without_content');
			return;
		}
